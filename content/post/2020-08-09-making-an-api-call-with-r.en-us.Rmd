...
output_df %>%
  mutate(date = lubridate::dmy(paste0("15-", month, "-14"))) %>%
  left_join(long_metric) %>%
  ggplot(aes(x = date, y = value, colour = long)) +
  scale_x_date(date_labels = "%b") +
  labs(x = "",
       y = "Value",
       colour = expression("Solar\ninsolation\n(kWh m"^{-2}*" day"^{-1}*")"),
       title = paste0("Monthly solar insolation data for lat ", lat, ", lon ", lon)) +
  geom_line() +
  theme_classic()
```
Cheers!
